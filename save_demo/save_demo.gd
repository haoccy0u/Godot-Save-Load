...
@onready var _save_manager = SaveManager
...
func _on_save_created(save_name: String, metadata: Dictionary):
	push_warning("存档已创建：%s，创建时间：%s" % [save_name, metadata.save_date])

## 存档加载回调
func _on_save_loaded(save_name: String, metadata: Dictionary):
	push_warning("存档已加载：%s，存档时间：%s" % [save_name, metadata.save_date])
	_update_player_display()

## 存档删除回调
func _on_save_deleted(save_name: String):
	push_warning("存档已删除：%s" % save_name)

## 自动存档创建回调
func _on_auto_save_created():
	push_warning("自动存档已创建")
	_update_save_list()
