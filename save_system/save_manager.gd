...
## @tutorial(自动加载): 需要在项目设置中将此脚本添加为自动加载，路径为 "res://save_system/save_manager.gd"，名称为 "SaveManager"
# 引用类型
...
## 可存档节点的组名
@export var save_group: String = "saveable"

## 默认存档格式
@export var default_format: String = "resource"
...
## 自动存档文件名前缀
@export var auto_save_prefix: String = "auto_save_"
...
## var _encryption_key: String = "123456"  # TODO: 暂时不考虑加密
...
func register_saveable_node(node: Node) -> void:
	if not node.is_in_group(save_group):
		...
		print_debug("注册存档节点: %s" % node.get_path())

	var node_path : String = node.get_path()
	# 检查是否有待加载的缓存状态
	if _pending_node_states.has(node_path):
		print_debug("发现节点 %s 的缓存状态，正在应用..." % node_path)
...
func create_auto_save() -> String:
	...
	if not success:
		push_error("创建自动存档失败：%s" % auto_save_id)
		return ""
	
	# 清理旧的自动存档
	var cleanup_success = await _clean_old_auto_saves()
	if not cleanup_success:
		push_warning("清理旧的自动存档失败")
...
func _collect_node_states() -> Array[Dictionary]:
	...
	for saveable in saveables:
		if saveable.has_method("save"):
			...
			push_warning("节点缺少save方法！%s" % str(saveable))
	return nodes
	...
	for node_data : Dictionary in nodes:
		...
		if node:
			if node.has_method("load_data"):
				...
				push_warning("节点缺少 load_data 方法！%s" % str(node))
		else:
			# 节点不存在，缓存数据
			print_debug("节点 %s 尚未加载，缓存其状态数据。" % node_path)
